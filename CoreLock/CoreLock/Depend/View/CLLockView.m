...
- (void)drawRect:(CGRect)rect {
    //数组为空直接返回
    if (!_itemViewsM || _itemViewsM.count == 0) return;
    ...
    //设置上下文属性
    //1.设置线条颜色
    [[UIColor colorWithHex:coreLockLockLineColor] set];
    //线条转角样式
...
- (void)layoutSubviews {
    ...
    CGFloat itemViewWH = (self.frame.size.width - 4 * CoreLockCellMargin) /3.0f;

    [self.subviews enumerateObjectsUsingBlock:^(UIView *subview, NSUInteger idx, BOOL *stop) {
        
        NSUInteger row = idx % 3;
        NSUInteger col = idx / 3;
        CGFloat x = CoreLockCellMargin * (row +1) + row * itemViewWH;
        CGFloat y = CoreLockCellMargin * (col +1) + col * itemViewWH;
...
- (void)setpwdCheck {
    ...
    if (count < CoreLockMinItemCount) {
		if (_setPWSErrorLengthTooShortBlock) {
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	switch (_type) {
		case CoreLockTypeSetPwd: {
			if (!self.firstRightPWD) {//第一次输入
				if (_setPWBeginBlock) _setPWBeginBlock();
			} else {//确认
				if (_setPWConfirmlock) _setPWConfirmlock();
			}
		} break;
			
		case CoreLockTypeVeryfiPwd: {
			if (_verifyPWBeginBlock) _verifyPWBeginBlock();
		} break;
			
		case CoreLockTypeModifyPwd: {
			if(_modifyPwdBlock) _modifyPwdBlock();
		} break;
